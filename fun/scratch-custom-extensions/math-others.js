...
function findReactComponent(element) {
  ...
  class ScratchMath {
    ...
    root_block({A, B}) {
      return Math.pow(new Number(B), (1 / new Number(A)));
    }
    negative_block({A}) {
      return 0 - new Number(A);
    }
    ...
    trunc2_block({A, B}) {
      let n = Math.floor(new Number(B));
      if (n >= 1) {
        n = 10 ** n;
        if (n !== Infinity) {
          return Math.trunc(new Number(A) * n) / n;
        }
        return new Number(A);
      }
      return Math.trunc(new Number(A));
    }
    trunc_block({A}) {
      return Math.trunc(new Number(A));
    }
    is_multiple_of_block({A, B}) {
      return new Number(A) % new Number(B) === 0;
    }
    log_with_base_block({A, B}) {
      return Math.log(new Number(A)) / Math.log(new Number(B));
    }
    pi_block() {
      return Math.PI;
    }
    ...
    is_safe_number_block({A}) {
      return Number.isSafeInteger(new Number(A));
    }
    is_number_block({A}) {
